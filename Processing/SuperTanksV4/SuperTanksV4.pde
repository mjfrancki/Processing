...
//////////////////level 3 setup
if (level == 3){
  BouncePadOne = new BouncePad(475, 750, 50, 10, 1);
 PowerMax = 15;
BouncePadOne.display();
BouncePadOne.bounce();
}
...
BouncePad(float xpos_, float ypos_, float padWidth_, float padHeight_,  float springiness_){
  
    xpos = xpos_;
    ypos = ypos_;
    padWidth = padWidth_;
    padHeight = padHeight_;
    springiness = springiness_;
...
void bounce(){
   ...
   if (PlayerOneTurn == true) {
    ...
    if((PlayerOne.bulletXpos > xpos) && (PlayerOne.bulletXpos < xpos + padWidth) && (PlayerOne.bulletYpos > ypos) && (PlayerOne.bulletYpos < ypos + padHeight)) {  
     PlayerOne.bulletYspeed = (PlayerOne.bulletYspeed* -1);
    }
   
  }

  if (PlayerOneTurn == false) {
    if((PlayerTwo.bulletXpos > xpos) && (PlayerTwo.bulletXpos < xpos + padWidth) && (PlayerTwo.bulletYpos > ypos) && (PlayerTwo.bulletYpos < ypos + padHeight)) {  
    PlayerTwo.bulletYspeed  =   (PlayerTwo.bulletYspeed*-1);
    }
  }
